...
SEARCH_BUDGET = 340       # 최악의 경우(전 노선 0건) 상한. 평소엔 150~200회다.
# 2026-09: 관심 노선을 3 → 18 개로 늘리며 260 → 340.
# 260 이면 최악의 경우 정확히 소진되어, 목록 뒤쪽(새로 넣은 탐색 노선)이
# 조용히 잘려 나갔다. 상한은 폭주를 막는 장치이지 평소 지출이 아니다.
# ★ 순서가 곧 우선순위다. main() 이 청주 → 출발지별 → 스위스 → 관심 노선
#   순으로 담으므로, 예산이 쪼들리면 새 탐색 노선이 먼저 잘린다.
